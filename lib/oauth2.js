...
function Strategy(options, verify) {
    ...
    options.authorizationURL = this.url + '/oauth2/login';
...
Strategy.prototype.registerClient = function (options) {
    ...
    return new Promise(function (resolve, reject) {
        self._oauth2._request(
            ...
            JSON.stringify({client_name: clientName, redirect_uri: self._callbackURL}),
            null,
            function (err, body) {
                if (err) {
                    debug(err);
...
Strategy.prototype.changePassword = function (options, done) {
    ...
    self._oauth2._request(
        ...
        function (err, body) {
            if (err) {
                debug(err);
                return done(new errors.NoPermissionError(err.message));
            }
